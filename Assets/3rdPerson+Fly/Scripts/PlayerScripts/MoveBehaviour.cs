...
public class MoveBehaviour : GenericBehaviour
{
	...
	public KeyCode duckingButton = KeyCode.LeftControl;
	...
	void Start()
	{
		...
		duckBool = Animator.StringToHash("Duck");
	...
	{
		// Get jump input.
		if (!duck && !jump && Input.GetButtonDown(jumpButton) && behaviourManager.IsCurrentBehaviour(this.behaviourCode) && !behaviourManager.IsOverriding())
		{
			jump = true;
		}

		if (!duck && !jump && behaviourManager.IsGrounded() && Input.GetKeyDown(duckingButton))
		{
			duck = true;
		}
		else if (duck && Input.GetKeyDown(duckingButton))
		{
			duck = false;
		}
	}
	...
	{
		// Start a new jump.
		if (jump && !behaviourManager.GetAnim.GetBool(jumpBool) && behaviourManager.IsGrounded() && !duck)
		{
	...
	{
		// Start a new jump.
		if (duck && behaviourManager.IsGrounded() && !behaviourManager.GetAnim.GetBool(duckBool))
		{
			// Set jump related parameters.
			behaviourManager.GetAnim.SetBool(duckBool, true);
		}
		else if (!duck && behaviourManager.GetAnim.GetBool(duckBool))
		{
			behaviourManager.GetAnim.SetBool(duckBool, false);
		}
